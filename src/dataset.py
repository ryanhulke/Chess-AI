...
def extract_rule_states(fen: str) -> tuple[int, int]:
    """Extract 5-fold repetition and 75-move rule states from FEN.
    
    Args:
        fen: The board position in Forsyth-Edwards Notation
        
    Returns:
        tuple of (repetition_state, move_rule_state) where:
        - repetition_state: 0 if < 3 repetitions, 1 if >= 3 repetitions
        - move_rule_state: 0 if < 50 moves, 1 if >= 50 moves
    """
    # Extract halfmove clock from FEN
    parts = fen.split(' ')
    halfmove_clock = int(parts[4])
    
    # For repetition, we can't determine from FEN alone, so we'll use a placeholder
    # In practice, you'd need to track game history
    repetition_state = 0  # Default to no repetition threat
    
    # 75-move rule: game ends at 75 moves without capture/pawn move
    # 50-move rule: can claim draw at 50 moves
    # We'll use 50 as threshold for "approaching draw"
    move_rule_state = 1 if halfmove_clock >= 50 else 0
    
    return repetition_state, move_rule_state
...
class ActionValueDataset(Dataset):
    ...
    def __init__(self, file_paths, hl_gauss=False, registers = 0, fraction=1.0, include_rule_states=False):
        ...
        self.include_rule_states = include_rule_states
        ...
        # Add 2 more tokens for rule states if enabled
        rule_tokens = 2 if include_rule_states else 0
        self.sample_sequence_length = SEQUENCE_LENGTH + 1 + registers + rule_tokens # (s) + (a) + (r) + rule_states
        ...
        self.vocab_size = BOARD_STATE_VOCAB_SIZE + NUM_ACTIONS + registers + (2 if include_rule_states else 0)

        self.action_offset = BOARD_STATE_VOCAB_SIZE
        self.register_offset = self.action_offset + NUM_ACTIONS
        self.registers = registers
        self.rule_states_offset = self.register_offset + registers if include_rule_states else None

        self.register_ids = None
        if registers:
            self.register_ids = np.arange(self.registers) + self.register_offset
            
        self.rule_state_ids = None
        if include_rule_states:
            self.rule_state_ids = np.arange(2) + self.rule_states_offset
    ...
    def _convert(self, sample):
        ...
        if self.include_rule_states:
            repetition_state, move_rule_state = extract_rule_states(fen)
            rule_tokens = np.array([
                repetition_state + self.rule_states_offset,
                move_rule_state + self.rule_states_offset + 1
            ], dtype=np.int32)
            sequence_arr.append(rule_tokens)
